...
exports.getAllSubCategories = AsyncHandler(async (req, res) => {
  ...
  // حساب عدد الأسئلة في جميع الامتحانات
  const subCategoryDetails = subCategories.map((subCategory) => {
    let totalQuestions = 0;

    subCategory.doors.forEach((door) => {
  ...
  res.status(200).json({
    results: subCategoryDetails.length,
    page,
    limit,
...
exports.getSubcategoryInCategory = AsyncHandler(async (req, res, next) => {
  ...
  // Calculate total questions for each subcategory
  const subCategoryDetails = subCategories.map((subCategory) => {
    let totalQuestions = 0;
...
exports.getSpecificSubCategory = AsyncHandler(async (req, res, next) => {
  ...
  const subCategory = await subCategoryModel.findById(id).populate({
    ...
    populate: {
      ...
      populate: [
        {
          path: "exams",
          select: "title image lesson duration questions", // Include questions
        },
        {
          path: "files",
          select: "title lesson",
  ...
  // Calculate total questions in all exams for this subcategory
  let totalQuestions = 0;
  subCategory.doors.forEach((door) => {
    door.lessons.forEach((lesson) => {
      lesson.exams.forEach((exam) => {
